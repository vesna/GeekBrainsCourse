...
import OOP_Homeworks.Units.Coordinates;
...
public class ConsoleView {
    ...
    public static void view() {
        ...
        for (int j = 1; j <= Main.GANG_SIZE; j++) {
            System.out.print(getChar(new Coordinates(j, Main.GANG_SIZE)));
        }
        System.out.print("|");
        System.out.println(PrintInfo(npcIndex));
    ...
    private static String getChar(Coordinates position) {
        ...
        for (int i = 0; i < Main.GANG_SIZE; i++) {
            if (Main.whiteSide.get(i).getCoordinates().isEquals(position)) {
                if (Main.whiteSide.get(i).getHp() == 0)
                    str = "|" + AnsiColors.ANSI_RED
                            + Main.whiteSide.get(i).getInfo().toUpperCase().charAt(0)
            ...
            if (Main.darkSide.get(i).getCoordinates().isEquals(position) && !alive) {
                if (Main.darkSide.get(i).getHp() == 0)
                    str = "|" + AnsiColors.ANSI_RED
                            + Main.darkSide.get(i).getInfo().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
                else
